 ...
 * from the actual UI beneath it). After the officer's chosen idle time (Settings) of no
...
fun SessionTimeoutMonitor(
    ...
    LaunchedEffect(active) {
        ...
        while (true) {
            ...
            val idleWarningAfterMs = com.pramaanai.officer.data.local.AppSettings.autoLockMinutes(context) * 60_000L
            if (idleFor >= idleWarningAfterMs) {
                showWarning = true
                val overshootSeconds = ((idleFor - idleWarningAfterMs) / 1000).toInt()
