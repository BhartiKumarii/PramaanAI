...
class LocalScreeningStore(context: Context) {
    ...
    /** Used when a queued-offline placeholder (temp id) is superseded by
     * the real record once the backend actually accepts the submission —
     * without this the placeholder would linger as a duplicate-looking
     * entry alongside the real one. */
